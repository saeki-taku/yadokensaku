...
export const HotelSimple = async (hotel_no: number) => {
    const res = await fetch(`${BASE_URL_SIMPLE}?format=json&hotelNo=${hotel_no}&applicationId=1081252385066817375`);

    return res.json();
};

export const HotelDetail = async (hotel_no: number) => {
    const res = await fetch(`${BASE_URL_DETAIL}?format=json&hotelNo=${hotel_no}&responseType=large&applicationId=1081252385066817375`);
    return res.json();
};
