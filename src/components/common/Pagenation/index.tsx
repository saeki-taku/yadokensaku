		...
		<div className="pagenation_wrap">
			...
			<div className="pagenation_box">
				{pagingInfo?.page !== 1 && pagingInfo?.pageCount > 5 && (
					<>
						<Link className="pagenation_btn _prev _start" href={`/search?keyword=${keyword}&page=1`}></Link>
						<Link className="pagenation_btn _prev" href={`/search?keyword=${keyword}&page=${pagingInfo?.page - 1}`}></Link>
					</>
				)}
				...
				<ul className="pagenation_numList">
					{list.map((num: number) =>
						calculatePaginationNumbers(num) ? (
							<li className={pagingInfo?.page === num ? "active" : ""} key={num}>
				...
				{pagingInfo?.pageCount !== pagingInfo?.page && pagingInfo?.pageCount > 5 && (
					<>
						<Link className="pagenation_btn _next" href={`/search?keyword=${keyword}&page=${pagingInfo?.page + 1}`}></Link>
						<Link className="pagenation_btn _next _end" href={`/search?keyword=${keyword}&page=${pagingInfo.pageCount}`}></Link>
					</>
				)}
			</div>
		</div>
